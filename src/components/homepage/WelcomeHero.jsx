...
export default function WelcomeHero({ user }) {
  ...
  const handleCreateEvent = () => {
    if (is_vendor) {
      ...
      // Show a message or modal instead of navigating
    ...
    <div className="rounded-2xl p-8 shadow-lg border border-blue-200">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div className="flex-1">
          ...
          {/* Show vendor upgrade message for non-vendors */}
          {!is_vendor && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <p className="text-sm text-yellow-800">
                <strong>Upgrade to Vendor:</strong> Create and manage events by upgrading your account to vendor status.
              </p>
            </div>
          )}
        </div>
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-6 py-3 bg-gradient-to-r ${
                  ...
                  isPriority ? "ring-2 ring-green-300 ring-offset-2 font-semibold text-base" : "text-sm"
                }`}
              >
